...
import { createMemoryStore, readAuditLog, resetMemoryStore } from './memory';

process.env.ENCRYPTION_KEY = 'test-encryption-key-1234';
...
async function main() {
  ...
  assert.equal(newest.total, 14);
  assert.ok(newest.items.every((item) => item.mediaUrl === null));
  ...
  assert.equal(afterFollow.total, 14);
  ...
  const saved = await store.updateProfile('usr_member', {
    ...
    orientation: 'queer',
    interests: ['music'],
    preferences: { showOnlineStatus: true, allowMessages: false, hideFromSearch: true },
  });
  ...
  assert.ok((creator?.contentCount ?? 0) >= 14);
