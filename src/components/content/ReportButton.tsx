...
export function ReportButton({ contentId }: { contentId: string }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      setMessage(body.alreadyReported ? 'You already reported this post.' : 'Report sent.');
      setOpen(false);
    } catch (cause) {
  ...
  return (
    <div>
      ...
      {open ? (
        <form onSubmit={(event) => void submit(event)} className="mt-3 space-y-3 rounded-2xl border border-zinc-800 bg-zinc-900 p-4">
          ...
          <button
            type="submit"
            disabled={pending}
            className="rounded-full bg-rose-600 px-4 py-1.5 text-sm font-medium text-white disabled:opacity-60"
          >
