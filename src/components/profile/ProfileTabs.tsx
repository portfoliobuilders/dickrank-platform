      ...
      <div role="tablist" aria-label="Profile sections" className="mb-6 flex gap-2">
        {TABS.map((item) => (
          <button
            ...
            onClick={() => setTab(item)}
            className={`rounded-full px-4 py-2 text-sm ${tab === item ? 'bg-zinc-100 text-zinc-950' : 'bg-zinc-900 text-zinc-300'}`}
          >
            {item}
          </button>
        ))}
        ...
        <div className="space-y-4 rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
          ...
          <dl className="grid gap-3 text-sm sm:grid-cols-2">
            ...
            <div>
              ...
              <dd className="capitalize">{profile.orientation?.replaceAll('_', ' ') || 'Not shared'}</dd>
            </div>
          </dl>
          <div>
