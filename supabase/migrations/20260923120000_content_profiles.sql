...
create index if not exists users_age_verification_idx on public.users (age_verification);
...
create index if not exists content_like_count_idx on public.content (like_count desc);
...
create or replace function public.toggle_content_like(p_content_id text, p_user_id text default null)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  if p_user_id is not null then
    ...
    from public.users
    where id = p_user_id and age_verification = true;
  else
    select id into v_user_id
    from public.users
  ...
  if exists (
    ...
    where user_id = v_user_id and content_id = p_content_id
  ) then
    delete from public.likes where user_id = v_user_id and content_id = p_content_id;
    update public.content set like_count = greatest(like_count - 1, 0) where id = p_content_id;
    v_liked := false;
  else
    insert into public.likes (user_id, content_id) values (v_user_id, p_content_id);
    update public.content set like_count = like_count + 1 where id = p_content_id;
    v_liked := true;
  end if;

  select like_count into v_count from public.content where id = p_content_id;
  return jsonb_build_object('liked', v_liked, 'likeCount', v_count);
end;
$$;

grant execute on function public.toggle_content_like(text, text) to authenticated, service_role;
